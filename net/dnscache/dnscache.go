...
func (r *Resolver) LookupIP(ctx context.Context, host string) (ip, v6 netip.Addr, allIPs []netip.Addr, err error) {
	...
	if ip := net.ParseIP(host); ip != nil {
		if ip4 := ip.To4(); ip4 != nil {
			return ip2addr(ip4), zaddr, []netip.Addr{ip2addr(ip4)}, nil
		}
		if debug {
			log.Printf("dnscache: %q is an IP", host)
		}
		return ip2addr(ip), zaddr, []netip.Addr{ip2addr(ip)}, nil
	}
...
func ip2addrs(n []net.IPAddr) []netip.Addr {
	ips := make([]netip.Addr, 0, len(n))
	for _, ip := range n {
		if len(ip.IP) == 4 {
			ips = append(ips, netip.AddrFrom4(*(*[4]byte)(ip.IP)))
		} else {
			ips = append(ips, netip.AddrFrom16(*(*[16]byte)(ip.IP)).WithZone(ip.Zone))
		}
	}
	return ips
}
...
func (r *Resolver) lookupIP(host string) (ip, ip6 netip.Addr, allIPs []netip.Addr, err error) {
	...
	defer cancel()
	ipsn, err := r.fwd().LookupIPAddr(ctx, host)
	if err != nil || len(ipsn) == 0 {
		if resolver, ok := r.cloudHostResolver(); ok {
			ipsn, err = resolver.LookupIPAddr(ctx, host)
		}
	}
	ips := ip2addrs(ipsn)
